...
(implicit executionContext: ExecutionContext) extends BaseController {
  ...
  def fetchContactData() = Action.async{ implicit request: Request[AnyContent] =>
    homeService.getAllContacts.map(contacts => Ok(contacts.mkString("\n")))
  }
    ...
    updateContactForm.bindFromRequest().fold(
      ...
      updatedContact => {
        if (updatedContact.phoneNumber.isEmpty && updatedContact.email.isEmpty) {
          ...
          homeService.updateContact(updatedContact.id, updatedContact).map(_ => Ok("Contact updated"))
        }
      })
    }
  ...
  def deleteContact(id: UUID) =
    Action.async { implicit request =>
      homeService.deleteContact(id).map { 
        case Future(1) => Ok("Contact deleted")
        case Future(@@) => BadRequest("Contact does not exist")
      }
    }
